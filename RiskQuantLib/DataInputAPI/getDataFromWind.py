...
def getOptionMarketData(securityString,tradeDateString):
	date = pd.Timestamp(tradeDateString)
	windData = w.wss(securityString, "sec_name,exe_mode,exe_price,exe_enddate,close","tradeDate="+date.strftime("%Y%m%d")+";priceAdj=U;cycle=D", usedf=True)
	tmp = windData[1].fillna(np.nan)
	tmp['EXE_ENDDATE'] = [i.strftime("%Y/%m/%d %H:%M") for i in tmp['EXE_ENDDATE']]
	tmp['DATE'] = date
	return tmp[['DATE']+tmp.columns[:-1].to_list()]

def getSHIBOR(tradeDateString):
	date = pd.Timestamp(tradeDateString)
	windData = w.wss("SHIBORON.IR,SHIBOR1W.IR,SHIBOR2W.IR,SHIBOR1M.IR,SHIBOR3M.IR,SHIBOR6M.IR,SHIBOR9M.IR,SHIBOR1Y.IR", "close","tradeDate="+date.strftime("%Y%m%d")+";priceAdj=U;cycle=D",usedf = True)
	return windData[1].fillna(np.nan)
